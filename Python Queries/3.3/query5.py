...
sql = ("""SELECT Year(date) as y, SUM(sales) FROM mrts
WHERE id = "45111" AND date LIKE "%-01-%" AND date NOT LIKE "%2021-%"
GROUP BY y;""")
cursor.execute(sql)
...
# The query is written
sql = ("""SELECT Year(date) as y, SUM(sales) FROM mrts
WHERE id = "45112" AND date LIKE "%-01-%" AND date NOT LIKE "%2021-%"
GROUP BY y""")
cursor.execute(sql)
...
# The query is written
sql = ("""SELECT Year(date) as y, SUM(sales) FROM mrts
WHERE  id = "451211" AND date LIKE "%-01-%" AND date NOT LIKE "%2021-%"
GROUP BY y;""")
cursor.execute(sql)
...
plt.title("Yearly Sales 1992 - 2020")
